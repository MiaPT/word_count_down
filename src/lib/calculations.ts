...
export function wordsRemainingToday(project: WritingProject){
    ...
    const writtenToday = project.entries.filter(e => areDatesEqual(e.date, today)).map(e => e.diff).reduce((total, current) => total + current, 0)
    return Math.max(Math.round((remainingTotal + writtenToday)/daysLeft(project)) - writtenToday, 0)
